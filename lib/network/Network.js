...
export function Network(container, data, options) {
  ...
  console.log("data in network");
  console.log(data);

  // Set levels
  /*for (let i in this.body.nodes) {
    let newNode = {Id: this.body.nodes[i].id, lvl: this.body.nodes[i].lvl};
    if (!nds.push) {
  ...
  data.nodes.lvls.forEach((obj) => {
    let newNode = {id: obj.id, lvl: obj.lvl};
    if (!nds.push) {
      console.log("Nodes are empty");
      nds = [];
    }
